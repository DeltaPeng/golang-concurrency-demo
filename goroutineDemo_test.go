// testing requires testing package
// suffix _test
// go test *_test.go
package main_test
...
func TestPrintHelloWorld_Validate(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		println("---valid, testing starting...---")
		...
		println("------valid, testing completed no issues!------\n")
	})

	t.Run("Invalid", func(t *testing.T) {
		println("---invalid, testing starting...---")
		for _, f := range []string{
			`hullo`,
			`world`,
			`huzzzah 2024`,
		} {
			println("invalid, testing: ",f)
			if (f != "2024"){
				err := "13"
		...
		println("------invalid, testing completed no issues!------\n")
	})
}


//temporary mit code below for test code review purposes, golang scim v2, 
